...
async function sync() {
  console.log("🚀 Starting Firestore Categories Sync...");
  for (let i = 0; i < CATEGORIES.length; i++) {
    const cat = CATEGORIES[i];
    const catData = { ...cat, order: i }; // Preserve order
    try {
      await setDoc(doc(db, "categories", cat.id), catData, { merge: true });
      console.log(`✅ [${i+1}/${CATEGORIES.length}] Synced & Merged: ${cat.name}`);
    } catch (e) {
      console.error(`❌ Error syncing ${cat.name}:`, e);
    }
  }
  console.log("✨ Firestore Safe Sync Complete!");
  process.exit(0);
}
